...
layui.define(['element',"fsConfig","fsCommon"], function(exports){
	...
	var menuConfig = {
			dataType : "server" , //
			loadUrl : "/tw_epm/actions/adminPublic_getMenu.action", //加载数据地址获取数据方式，local本地获取，server 服务端获取
			rootMenuId : "0", //根目录菜单id
			defaultSelectTopMenuId : "1", //默认选中头部菜单id
			defaultSelectLeftMenuId : "home", //默认选中左边菜单id
			...
			data : [
				...
				{"menuId":"home","menuName":"首页","menuIcon":"&#xe68e;","menuHref":"views/home/index.html","parentMenuId":"1"},
		...
		if(!$.isEmpty(data)){
			...
			$.each(data,function(i,v){
				if(menuConfig.rootMenuId === v[menuConfig.parentMenuIdField]){
					...
					topStr += '" dataPid="'+v[menuConfig.menuIdField]+'" lay-id="'+v[menuConfig.menuIdField]+'" menuid="'+v[menuConfig.menuIdField]+'"><a href="javascript:;">'+thisMenu.getIcon(v[menuConfig.menuIconField])+' <cite>'+v[menuConfig.menuNameField]+'</cite></a></li>';
